...
long get(List L){
	...
	if (L->numItems == 0) {
		printf("Error in module List, get() cannot be called on an empty list\n");
		exit(1);
	}
	if (L->index < 0) {
...
void set(List L, long x) {
	if (L->cursor == NULL || L->index == -1) {
		printf("Error: cannot call set on an undefined cursor element\n");
		exit(1);
	}
	if (L->numItems == 0) {
		printf("Error: cannot call set on an empty List\n");
		exit(1);
	}
	L->cursor->data = x;
}
